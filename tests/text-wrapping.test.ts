  ...
  for (const original of plans) {
    ...
    for (const mediaKind of ["image", "carousel"] as const) {
      ...
      for (let slide = 0; slide < plan.slideCount; slide++) {
        ...
        const blocks = [
          ...svg.matchAll(
            /<g data-feed-text="true" data-x="([\d.]+)"[^>]*font-family="(Manrope|Figtree)" font-size="([\d.]+)" font-weight="(\d+)" aria-label="([^"]*)"/g,
          ),
        ];
        ...
        for (const match of blocks) {
          ...
          const ink = outlineText({
            bytes: match[2] === "Manrope" ? brand.manrope : brand.figtree,
            text,
            size: Number(match[3]),
          ...
          assert.ok(
            x + ink.right - Math.min(0, ink.left) <= 1040,
